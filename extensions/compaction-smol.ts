...
export default function (pi: ExtensionAPI) {
	pi.on("session_before_compact", async (event, ctx) => {
		...
		// Use GPT-5.6 Luna for summarization (cheaper/faster than the main conversation model)
		const model = ctx.modelRegistry.find("openai-codex", "gpt-5.6-luna");
		if (!model) {
			ctx.ui.notify(`Could not find GPT-5.6 Luna model, using default compaction`, "warning");
			return;
		}

		// Resolve request auth for the summarization model. Codex login supplies
		// authorization headers rather than an API key.
